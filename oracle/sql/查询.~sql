...
--查询名字中带%的用户
--转义字符可以自己定义
SELECT * from emp where ename like('ca.%%') escape('.');
select * from emp where ename like('%\%%') escape('\');

select empno,sal,ename from emp where ename like '%S%';
...
select ename,(e.sal+nvl(e.comm,0))*12 年薪 from emp e;
